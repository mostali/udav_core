...
//	}
	...
	public static String fnp(Path path, String... defRq) {
		if (path != null) {
			...
			return ARG.toDefThrow(() -> new RequiredRuntimeException("Path parent is null"), defRq);
		}
	...
	public static String fn(Path resolve, int i, String... defRq) {
		if (i > resolve.getNameCount()) {
			return ARG.toDefThrowMsg(() -> X.f("Not found path item %s from %s", i, resolve), defRq);
		}
		return fn_(resolve, i).toString();
	}

	public static Path fn_(Path resolve, int i) {
		Path path = null;
		for (int f = resolve.getNameCount() - i; f < resolve.getNameCount(); f++) {
